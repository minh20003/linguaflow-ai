...
| Bảng | Các trường |
|---|---|
| `users` | `id`, `email`, `username`, `display_name`, `password_hash`, `google_sub`, `role`, `preferred_language`, `interface_language`, `timezone`, `created_at` |
...
21. `users.timezone` là tên IANA (`Asia/Ho_Chi_Minh`), do trình duyệt báo lên một lần mỗi phiên qua `PUT /auth/me/timezone`, và **cho phép rỗng**. Nó tồn tại vì một mốc giờ nói trong hội thoại — "3 giờ chiều thứ Sáu" — là **giờ treo tường**, không phải một thời điểm: thiếu độ lệch múi giờ thì không quy đổi được sang UTC. `normalize_action_time` cố ý **từ chối lấy múi giờ từ đầu ra của model** (`trusted_timezone` chỉ nhận dữ liệu đã xác thực), vì một độ lệch đoán sai sẽ đặt cuộc họp lệch giờ mà không có gì báo. Trước khi có cột này máy chủ không có nguồn đáng tin nào, nên **mọi** thời gian trích xuất được đều rơi vào `missing_fields` và người dùng phải gõ lại lúc duyệt. Lưu **tên vùng** chứ không lưu độ lệch: độ lệch chỉ đúng tới lần đổi giờ mùa kế tiếp, nên một cuộc hẹn đặt tháng 10 cho tháng 12 sẽ lệch một tiếng. Rỗng vẫn hợp lệ — một tài khoản chưa từng mở giao diện web thì không có trình duyệt nào báo, và bước duyệt vẫn hỏi như cũ; cột này bỏ được trường hợp phổ biến chứ không bỏ đường lui.
...
24. **`display_title` / `display_details` là trường chỉ đọc, không thay `title`.** Hộp nhiệm vụ và Lịch cá nhân là chrome nên chữ trên đó theo `interface_language`, trong khi `action_proposals.title` và `calendar_events.title` được lưu theo **ngôn ngữ dịch** của chủ sở hữu (ghi chú 22). Hai cài đặt này thường trùng nhau; khi khác nhau, `GET /me/action-proposals` và `GET /me/calendar/events` trả thêm bản đã dịch ở `display_title` (và `display_details` cho đề xuất), còn `title` **luôn là giá trị đã lưu**.

Tách hai trường chứ không ghi đè `title`, vì **client gửi `title` ngược lên**: form sửa trong hộp nhiệm vụ điền từ chính phản hồi đó và `decisionCorrections` luôn đính kèm `title`, còn Lịch cá nhân `PATCH title` mỗi lần kéo thả. Bản trả về có `title` đã dịch nghĩa là chỉ cần bấm Duyệt mà không sửa gì, **bản dịch máy ghi đè lên câu người dùng thật sự nói** — và mỗi lần sửa sau lại dịch chồng lên. Không có gì phía sau phát hiện được điều đó.

Ba ràng buộc đi kèm. Thứ nhất, **mọi endpoint trả về một đề xuất** (`confirm`, `reject`, `clarify`, `dismiss`) đều điền `display_*`, nếu không thì duyệt xong hàng thô đẩy ngược vào danh sách và tiêu đề đổi ngôn ngữ giữa chừng. Thứ hai, **lịch chỉ dịch sự kiện `source = 'assistant'`** — tiêu đề người dùng tự gõ hoặc đồng bộ từ Google không nằm ở ngôn ngữ dịch, đưa vào bộ dịch với sai `source_language` thì trả về méo. Thứ ba, số dòng được dịch **có trần** và chạy song song: trang lịch xin toàn bộ sự kiện, mà mỗi lần gọi bộ dịch dự phòng có timeout 5 giây — tuần tự thì đó là hàng phút treo request trên deployment một tiến trình (ADR-18). Quá trần thì giữ nguyên văn đã lưu, giống hệt cách một lần gọi thất bại được xử lý.

`AssistantState.reply_language` **không thuộc contract** — nó là khoá nội bộ của đồ thị trợ lý, cùng lý do ADR-16 nêu cho `telemetry`, và chỉ dùng cho mấy câu trả lời cố định khi model không sinh ra gì. Câu trả lời thật đi theo ngôn ngữ của câu hỏi.

23. **Phạm vi đọc của Assistant Agent do nơi phát sinh yêu cầu quyết định**, không do câu hỏi. Hai phạm vi, và `src/services/assistant_scope.py` là nơi duy nhất chọn giữa chúng.

`personal` — hội thoại riêng với trợ lý (nhận diện bằng `conversations.title = '__linguachat_assistant__'`). Ở đây trợ lý đọc được **mọi hội thoại mà tài khoản đó là thành viên**, kèm **danh sách thành viên** của chúng. Đây là điều kiện cần để trả lời được những câu như "liệt kê tất cả cuộc hẹn sắp tới của mình": các cuộc hẹn nằm rải ở đúng những luồng đã hẹn nhau, còn luồng riêng với trợ lý thì không chứa cái nào — nên một trợ lý bị giới hạn trong luồng nó đang đứng sẽ luôn trả lời "không tìm thấy gì" bất kể tài khoản có bao nhiêu.

`conversation` — `@assistant` hoặc trả lời trong một hội thoại cụ thể. Chỉ đọc **đúng hội thoại đó**. Câu hỏi được đặt ra tại chỗ, và câu trả lời được sinh ra trong một phòng có người khác.

Nhận diện bằng **tiêu đề** chứ không bằng hình dạng "nhóm một thành viên": hình dạng đó tồn tại tự nhiên khi mọi người rời nhóm, và một nhóm rỗng dần **không được** âm thầm trở thành cửa sổ nhìn vào mọi hội thoại của người còn lại. Không xác định được thì rơi về `conversation` — mở rộng khi thiếu phụ thuộc là cách một chốt quyền trở thành trang trí.

Quy tắc này **khác** với quy tắc về người được đọc câu trả lời. Trả lời cho một `@assistant` là tin nhắn `private` chỉ người hỏi thấy (ghi chú 20, ADR-31); cái đó nói **ai đọc được câu trả lời**, còn cái này nói **trợ lý được đọc gì để viết ra nó**. Cả hai đúng cùng lúc.

Hệ quả với mã: `assistant_retrieval.retrieve` nhận `conversation_ids` (một tập) chứ không phải một id, và tập rỗng trả về rỗng chứ không phải tìm tất cả. `chunk_index` đánh số lại từ 0 trong mỗi hội thoại, nên khi mở rộng child→parent phải khoá theo `(conversation_id, chunk_index)`; chỉ dùng chỉ số sẽ ghép một chunk với parent của hội thoại khác. Các công cụ vốn đã theo tài khoản (`list_calendar_events`, bộ nhớ cá nhân) không đổi — chúng chưa bao giờ bị giới hạn theo hội thoại.

22. `action_proposals.owner_user_id` là **người được hỏi**, không phải người đã nói ra câu đó. Với `source_mode = 'proactive'` — trợ lý tự quét và tự nhận ra — **mỗi thành viên trong hội thoại được một hàng riêng của mình**, cùng `source_message_id`, cùng nội dung, nhưng `status` độc lập. Một cuộc hẹn là việc của tất cả những người có mặt, nên tất cả đều được hỏi; và **duyệt hay từ chối là quyền riêng của từng người** — một hàng dùng chung không diễn đạt được điều đó, vì một cột `status` duy nhất không nói được "hai người đã duyệt, người thứ ba từ chối". Thành viên khác chỉ được tạo hàng khi **chính họ** đã cấp `proactive_scan`: không ai hỏi hộ họ cả, và không có hàng quyền nghĩa là chưa cấp (ghi chú 19). Riêng người gửi luôn có hàng, vì lối vào đã kiểm tra quyền cho họ rồi — endpoint thủ công chỉ đòi `read_conversations`, còn trình quét nền đã kiểm `proactive_scan` trước khi gọi.

Ngược lại, `source_mode = 'on_demand'` — người dùng `@assistant` hoặc bấm nút — **chỉ tạo một hàng cho chính người hỏi**. Câu trả lời cho một người có thể tóm tắt cam kết của người khác, và đẩy nó cho cả nhóm là tiết lộ về những thành viên không hề yêu cầu (ADR-31, cùng lý do với `messages.visibility`). Hai lối đi này kết thúc ở **cùng một bảng và cùng một thẻ giao diện**, nên rất dễ vô tình cho chúng chung một quy tắc hiển thị: phải hỏi "hàng này do lối nào sinh ra" trước khi quyết định ai được thấy.

Mọi hàng của cùng một tin nhắn đều quy đổi thời gian theo **múi giờ của người nói**, không phải của chủ hàng. "6h" là giờ treo tường thuộc về người phát ngôn; đọc lại nó trên đồng hồ của từng người nhận sẽ dời cuộc hẹn đi đúng bằng độ lệch múi giờ, và tạo ra cảnh mấy thành viên giữ cùng một cuộc hẹn ở những khoảnh khắc khác nhau — tệ hơn hẳn việc hỏi lại. Đó là ý nghĩa của tham số `timezone_user_id` trong `create_proposals_from_candidates`.
