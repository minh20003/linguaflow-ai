...
@pytest.mark.asyncio
async def test_stats_on_an_empty_database_returns_zeroes(client, test_user_headers):
...
async def test_stats_counts_outcomes_that_produced_no_translation(
    client, test_db, test_user, test_user_two, conversation_factory, test_user_headers
):
    ...
    body = (await client.get("/api/v1/stats", headers=test_user_headers)).json()
...
async def test_days_limits_the_window(
    ...
    body = (await client.get("/api/v1/stats?days=7", headers=test_user_headers)).json()
...
@pytest.mark.asyncio
async def test_an_unbounded_window_is_rejected(client, test_user_headers):
    """Bounded so one request cannot ask for an unlimited history."""
    response = await client.get("/api/v1/stats?days=99999", headers=test_user_headers)
